...
sq_dbem_grid %>%
  select(f_finale) %>%
  write.table(file = paste0(data_path,"spatial/compute_canada/mpa_sq_update.txt"),
              sep="\t", 
              col.names = F, 
              row.names = F)
...
ri_dbem_grid <- dbem_grid %>% 
  left_join(scen_reg_implement,
            by ="index") %>% 
  select(index,lon,lat,status,mpa_abrev,status,scen,f_scen,fe_prop) %>% 
  mutate(
    # f_finale_plot = ifelse(f_finale > 1, f_finale, f_finale*-1),
...
rpc_dbem_grid <- dbem_grid %>% 
  left_join(scen_reg_implement,
            by ="index") %>% 
  select(index,lon,lat,status,mpa_abrev,status,scen,f_scen,fe_prop,f_out) %>% 
  mutate(
    # f_finale_plot = ifelse(f_finale > 1, f_finale, f_finale*-1),
    f_finale = ifelse(is.na(status) & index %in% etp_eez_index_ri$index, 1.3,
                      ifelse(is.na(status) & ! index %in% etp_eez_index_ri$index,1,
...
ggsave(
       paste0(data_path,"../manuscripts/scenarios_paper/results/scenarios/rpc_scenario_mapb.png"),
       rpc_map)
