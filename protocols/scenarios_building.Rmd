...
mpas_grid %>% 
  filter(mpa_abrev == "Hermandad_Ecuador") %>% 
  left_join(dbem_grid) %>% 
  ggplot() +
  geom_tile(
    aes(
...
etp_eez_index_sq <- my_data("sau_index") %>% 
   filter(eez_name %in% c("Costa Rica (Pacific)","Colombia (Pacific)","Panama (Pacific)","Galapagos Isl. (Ecuador)", "Ecuador")) %>% 
  select(index) %>% 
  filter(!index %in% sq_dbem_grid$index) %>% 
  # mutate(rfmo_name = "IATTC") %>% 
  mutate(f_finale = 1.3,
         status = "unprotected",
         mpa_abrev = NA,
...
sq_map <- sq_dbem_grid %>% 
  ...
  scale_fill_viridis_c() +
  scale_color_viridis_c() +
  geom_sf(data = combined_sf %>% filter(FID != 8), aes(), fill = "transparent", color = "black")  +
...
ggsave(
       paste0(results_path,"/scenarios/sq_scenario_mapz.png"),
       sq_map)


```

# Scenario 2: Best Case (regulation implemented)
...
ri_dbem_grid <- dbem_grid %>% 
  left_join(scen_reg_implement,
            by ="index") %>% 
  select(index,lon,lat,status,mpa_abrev,status,scen,f_scen) %>% 
  mutate(
    # f_finale_plot = ifelse(f_finale > 1, f_finale, f_finale*-1),
    f_finale = ifelse(is.na(status) & index %in% etp_eez_index_ri$index, 1.3,
                      ifelse(is.na(status) & ! index %in% etp_eez_index_ri$index,1, f_scen)
    )
  ) %>%
  distinct(index, .keep_all = T)
...
ggsave(
       paste0(data_path,"spatial/compute_canada/ri_scenario_map.png"),
       ri_map)
...
# Scenario 3: Partial regulations For conservation (reg_par_con)
...
scen_reg_implement <- sq_mpa_grid_f %>% 
  mutate(f_rpc = ifelse(status == "protected",f_plan,f_scen))
...
rpc_dbem_grid <- dbem_grid %>% 
  ...
  mutate(
    # f_finale_plot = ifelse(f_finale > 1, f_finale, f_finale*-1),
    f_finale = ifelse(is.na(status) & index %in% etp_eez_index_sq$index, 1.3,
                      ifelse(is.na(status) & ! index %in% etp_eez_index_sq$index,1, f_scen)
    )
  ) %>%
  distinct(index, .keep_all = T)
...
ggsave(
       paste0(data_path,"spatial/compute_canada/rpc_scenario_mapb.png"),
       rpc_map)


```

# Scenario 4: Partial rgulations For fisheries (reg_par_pes)

The fishing area within the marine protected areas is 20% larger than the area specified in the management plan. Fishing intensity in the region is at the level required for MSY
...
scen_reg_implement <- sq_mpa_grid_f %>% 
  mutate(f_rpf = ifelse(status == "protected",f_scen,
                        ifelse(status == "surrounding",f_finale-f_out,f_plan))
  )
...
rpp_dbem_grid <- dbem_grid %>% 
  ...
  mutate(
    # f_finale_plot = ifelse(f_finale > 1, f_finale, f_finale*-1),
    f_finale = ifelse(is.na(status) & index %in% etp_eez_index_sq$index, 1,
                      ifelse(is.na(status) & ! index %in% etp_eez_index_sq$index,1, f_scen)
    )
  ) %>%
  distinct(index, .keep_all = T)
...
ggsave(
       paste0(results_path,"/scenarios/rpp_scenario_map.png"),
       rpp_map)


```

# Scenario 5: No regulations
...
scen_reg_implement <- np_mpa_grid_f %>% 
  mutate(f_nr = ifelse(status == "protected",f_scen,
                        ifelse(status == "surrounding",f_finale-f_out,f_plan))
  )
...
nr_dbem_grid <- dbem_grid %>% 
  ...
  mutate(
    ...
    f_finale = ifelse(is.na(status) & index %in% etp_eez_index_np$index, 1.5,
                      ifelse(is.na(status) & ! index %in% etp_eez_index_np$index,1, f_scen)
    )
  ) %>%
  distinct(index, .keep_all = T)
...
ggsave(
       paste0(results_path,"/scenarios/nr_scenario_map.png"),
       rpp_map)
