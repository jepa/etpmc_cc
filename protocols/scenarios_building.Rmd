...
mpas_grid %>% 
  filter(mpa_abrev == "CLomas Sub  Cuenca PacNor") %>% 
  left_join(dbem_grid) %>% 
  ggplot() +
  geom_tile(
    aes(
...
ri_dbem_grid <- dbem_grid %>% 
  left_join(scen_reg_implement,
            by ="index") %>% 
  select(index,lon,lat,status,mpa_abrev,status,scen,f_plan,f_scen,fe_prop,f_out) %>% 
  mutate(
    # f_finale_plot = ifelse(f_finale > 1, f_finale, f_finale*-1),
...
ggsave(
       paste0(data_path,"../manuscripts/scenarios_paper/results/scenarios/rpp_scenario_map.png"),
       rpp_map)
