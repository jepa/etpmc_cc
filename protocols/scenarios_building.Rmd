...
scenario_pannel <-
  bind_rows(
  sq_dbem_grid %>% mutate(scen = "1_stat_quo"),
  nr_dbem_grid %>% mutate(scen = "2_no_reg"),
  ri_dbem_grid %>% mutate(scen = "3_reg_imp"),
  rpp_dbem_grid %>% mutate(scen = "4_reg_par_fish"),
  rpc_dbem_grid %>% mutate(scen = "5_reg_par_cons")
) %>% 
  # View()
  ggplot() +
  geom_tile(
  ...
  scale_color_viridis_c() +
  geom_sf(data = sau_sf, aes(), fill = "transparent", color = "black")  +
  geom_sf(data = combined_sf, aes(), fill = "transparent", color = "grey30")  +
  my_land_map()+ 
  coord_sf(
    xlim = c(-95,-77),
    ylim = c(-4,9)
