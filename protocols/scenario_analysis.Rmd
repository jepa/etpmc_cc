...
lapply(scenarios, agg_esm_fx,category = "Abd")
  )
...
agg_average_data <- scenarios_data %>%
  ...
  write_csv(my_path("R","processed/all/period_data/","all_period_data_abd.csv"))
...
# Run plot making
mclapply(etpmc_spp$taxon_key[1], scen_dif_fx, var = "abd", mc.cores = detectCores() - 4)
mclapply(etpmc_spp$taxon_key, scen_dif_fx, var = "Catch", mc.cores = detectCores() - 4)



```
#### Ch. rel. to SQ, per group percentage change SQ
...
# Run plot making
lapply(unique(groups$group), scen_dif_fx,"Catch")
lapply(unique(groups$group), scen_dif_fx,"Abd")
...
map_name <- paste0("agg_catch_scen_diff_ssp585.png")
