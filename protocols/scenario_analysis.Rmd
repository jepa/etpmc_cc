...
cmar_grid <- my_data("dbem_coords") %>% 
  ...
  mutate(status = ifelse(is.na(status),"non_protected",status),
         mpa_abrev = ifelse(is.na(mpa_abrev),eez_name,mpa_abrev)
  ) %>% 
  filter(!mpa_abrev %in% c("Colombia (Caribbean)","Panama (Caribbean)","Costa Rica (Caribbean)","El Salvador","Nicaragua (Pacific)","Peru","Venezuela"),
         !is.na(eez_name))
    ...
    if(file.exists(file_to_read)){
      ...
      early_data <- spp_data %>% 
        ...
        mutate(
          ...
          scen = str_sub(file_to_read,88,89)
        )
...
ran_spp <- list.files(my_path("R","processed"))
...
scen <- my_path("R","Rdata",list_files = "paths")
