...
lapply(unique(spp_groups$group), agg_abd_group, variable = "catch", ignore_taxa = 600226)
lapply(unique(spp_groups$group), agg_abd_group, variable = "abd")

agg_abd_group(group_selected = "dorado",variable = "abd")
...
write_csv(scenarios_data,
          my_path("R","processed/all/period_data/","all_period_data_abd_esm.csv"))
...
```{r chec_spp, eval = F, echo = F}

# Check raw outputs
for(i in 1:length(etpmc_spp$taxon_key)){
  
  name <- paste0(etpmc_spp$taxon_key[i],"_abd_period.csv")
  
  spp_data <- my_path("R","processed/species/period_data/",name,read = T) %>% 
  ...
  nameplot <- paste0(etpmc_spp$common_name[i],"_bar.png")
  
  ggsave(
    my_path("R","figures/relative_to_hist/groups/",nameplot),
    last_plot(),
    height = 8,
    width = 9)
...
# Check raw outputs
for(i in 1:length(spp_groups_list)){
  
  name <- paste0(spp_groups_list[i],"_abd_period.csv")
  ...
  nameplot <- paste0(spp_groups_list[i],"_bar.png")
...
all_abd_data <- my_path("R","processed/all/period_data/",name = 'all_period_data_abd.csv',read = T)


all_abd_data <- read.csv(paste0(data_path,"04 raw dbem results/processed/all/all_years_data_abd_esm.csv"))

change_all_abd_data <- all_abd_data %>% 
  mutate(
    period = ifelse(year < 2060 & year > 2041,"2050_mid",
                    ifelse(year < 2040 & year > 2021,"2030_ear",
                           ifelse(year < 2014 & year > 1995,"2014_hist",
                                  NA)
                    )
    )
  ) %>%
  group_by(region,period,ssp,scen) %>%
  summarise(
    mean_value_esm = mean(value_region,na.rm = T),
    .groups = "drop"
  ) %>%  
  filter(!is.na(period)) %>% 
  pivot_wider(names_from = period, values_from = mean_value_esm) %>% 
  mutate(
    dif_2040 = round(my_chng(`2014_hist`,`2030_ear`),2),
    dif_2060 = round(my_chng(`2014_hist`,`2050_mid`),2)
  ) %>% 
  # select(
  #   category:ssp,dif_2040
  # ) %>% 
  scenario_names() %>% 
...
spp_groups <-  read_excel("~/Library/CloudStorage/GoogleDrive-jepa88@gmail.com/My Drive/lenfest_mpa_project/data/00 species/dbem_spp/dbem_spp_list.xlsx", 
                          skip = 1) %>% 
  filter(group != "mola",
         taxon_key != 600226)
...
# hist_dif_sq("dorado", var = "abd")
# Run plot making
for(i in 1:2){
  vars <- c("abd","mcp")[i]
  mclapply(etpmc_spp$taxon_key, hist_dif, var = vars, mc.cores = detectCores() - 4)
  mclapply(etpmc_spp$taxon_key, hist_dif_sq, var = vars, mc.cores = detectCores() - 4)
}
...
per_chng_groups_period <- groups_df %>% 
  ...
  pivot_wider(names_from = period, values_from = mean_value_esm) %>% 
  mutate(
    per_change = my_chng(`2014_hist`,`2040_ear`, limit = 50)
  ) %>% 
  select(
    scen,region, group, ssp,per_change
  ) %>% 
...
per_chng_groups_period %>% 
  ...
  scale_x_discrete(limits = rev(sort(spp_data %>% filter(region != "High seas") %>% mutate(region = gsub("_"," ",region)) %>% pull(region) %>% unique())))



ggsave(
  my_path("R","figures/relative_to_hist/groups/","x_groups_facet_bar_ssp126_time_2040.png"),
  last_plot(),
  height = 8,
  width = 9)
...
data_paths <- my_path("R","processed/all/period_data/",list_files = "paths",fn = T)
...
spp_data <- agg_data %>% 
  ...
  pivot_wider(names_from = period, values_from = mean_value_esm) %>% 
  mutate(
    dif_2040 = my_chng(`2014_hist`,`2060_mid`)
  ) %>% 
  select(
    scen:ssp,dif_2040
  ) %>% 
...
map_t <- ggplot() +
  ...
  facet_wrap(~scenario) +
  scale_fill_gradient2("Change in biomass relative to 2014 (%)") +
...
map_name <- paste0("map_",category_selected,"_",ssp_selected,"_time_diff_2060.png")
...
delta_delta <- spp_data %>% 
  filter(category == category_selected,
         ssp == ssp_selected) %>% 
  select(region,scen,dif_2040) %>% 
...
delta_delta %>% 
  ...
  coord_flip() +
  labs(x="",
       y = "Percentage difference between management scenarios and the SQ (%)")+
  my_ggtheme_p(leg_pos = "right") +
  scale_x_discrete(limits = rev(sort(delta_delta %>% mutate(region = gsub("_"," ",region)) %>% pull(region) %>% unique())))

names <- paste0("bar_delta_all_",category_selected,"_",ssp_selected,".png")

ggsave(
  my_path("R","figures/relative_to_hist/delta_delta/",names),
  last_plot(),
  height = 5,
  width = 7)
...
for(s in 1:2){
  for(p in 1:2){
    ...
    ssp_selected <- c("ssp126","ssp585")[s]
    category_selected <- "Abd"
    period_selected <- c("2060_mid","2040_ear")[p]
    
    map_t <- ggplot() +
      geom_sf(data = sau_sf %>% select(region = name) %>%  
                left_join(spp_data) %>% 
                filter(ssp == ssp_selected, 
                       category == category_selected,
      ...
      geom_sf(data = cmar_sf %>%  left_join(spp_data) %>% filter(ssp == ssp_selected,category == category_selected, period == period_selected),
              aes(fill = per_change),
              color = "black"
      ) +
      facet_wrap(~scenario) +
    ...
    map_name <- paste0(category_selected,"_",ssp_selected,"_",period_selected,"_time_diff.png")
    ...
    bar_name <- paste0("bar_",category_selected,"_",ssp_selected,"_",period_selected,"_time_diff.png")
...
Statistical uncertainty using anova
...
library(purrr)
library(broom)
# Function to compute percent variance explained by each factor
get_var_contrib <- function(df) {
  if (n_distinct(df$ssp) > 1 & n_distinct(df$esm) > 1) {
    model <- aov(mean_value_tp ~ esm + ssp, data = df)
    anova_tbl <- broom::tidy(model)
    
    total_ss <- sum(anova_tbl$sumsq, na.rm = TRUE)
    
    anova_tbl %>%
      filter(term %in% c("esm", "ssp")) %>%
      mutate(perc_var = 100 * sumsq / total_ss)
  } else {
    # Return NA if not enough variation
    tibble(term = c("esm", "ssp"), sumsq = NA, df = NA, statistic = NA, p.value = NA, perc_var = NA)
  }
}


uncertainty_by_region_period <- scenarios_data %>%
  filter(scen == "sq") %>% 
  #for aggregated
  group_by(esm, period, ssp) %>%
  summarise(mean_value_tp = sum(mean_value_tp,na.rm = T)) %>% 
  # View()
  group_by(period) %>%
  group_modify(~ get_var_contrib(.x)) %>%
  ungroup()


ggplot(uncertainty_by_region_period, aes(x = period, y = perc_var, fill = term)) +
  geom_col(position = "stack") +
  # facet_wrap(~ region, scales = "free_x") +
  labs(
    y = "Proportion of Uncertainty (%)",
    x = "Period",
    fill = "Source of Uncertainty",
    title = "Uncertainty Contribution by Region and Period"
  ) +
  theme_minimal() +
  theme(
