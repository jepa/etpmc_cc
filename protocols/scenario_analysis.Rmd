...
agg_fx <- function(scenario, category){
  ...
  final_df <- complete_output %>% 
    mutate(
      ...
      scenario = str_sub(scenario,10,12)
    )
...
sqb_scen_df <- agg_fx("c6gfdl26F1nr", "Abd")
...
ggsave(
  paste0(results_path,"/scenario_map_nr.jpg"),
  map,
  width = 14)
...
ggsave(
  paste0(results_path,"/status_boxplot_nr.jpg"),
  last_plot()
)
...
ggsave(
  paste0(results_path,"/mpa_boxplot_nr.jpg"),
  last_plot(),
  width = 14)
...
scenarios <- list.files(my_path("R"))[-6]
