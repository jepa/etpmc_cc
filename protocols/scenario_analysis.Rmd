...
mclapply(etpmc_spp$taxon_key, spp_fx, variable = "abd", mc.cores = detectCores() - 4)
...
lapply(etpmc_spp$taxon_key, scen_dif_fx,"abd")
...
scen_group_dif_fx <- function(variable = "abd",grupo){
  ...
  if(variable == "abd"){
    ...
    name <- paste0(selected_group$taxon_key,"_",variable,".RData")
  }
  ...
  final_output <- complete_output %>% 
      left_join(cmar_grid,
                  by = "index",
                  relationship = "many-to-many") %>% 
    group_by(period,year,region,esm,group,ssp) %>% 
    #aggregate all grids
    summarise(
    ...
    group_by(period,region,group,esm,ssp) %>% 
    summarise(
      mean_value_tp = mean(sum_v, na.rm = T),
      .groups = "keep"
    ) %>% 
    # average per ESM
    group_by(period,region,group,ssp) %>% 
    summarise(
...
lapply(unique(groups$group), scen_group_dif_fx, variable = "abd")
