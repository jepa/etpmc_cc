...
scenario_pannel <-
  ...
  rpp_dbem_grid %>% mutate(scen = "Regulations for fishing (RF)"),
  rpc_dbem_grid %>% mutate(scen = "Regulations for conservation (RC)")
) %>% 
  # View()
  ggplot() +
  geom_tile(
  ...
  geom_sf(data = sau_sf, aes(), fill = "transparent", color = "grey30")  +
  geom_sf(data = combined_sf, aes(), fill = "transparent", color = "black")  +
  my_land_map()+ 
