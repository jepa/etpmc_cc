...
sf_list <- lapply(seq_along(path_mpa), function(i) {
  sf_obj <- st_read(path_mpa[i])
  sf_obj$mpa_name <- mpa_name[i]  # Add the corresponding MPA name
  return(sf_obj)
})
...
# Ensure 'mpa_name' is included in the final dataset
if (!"mpa_name" %in% names(combined_sf)) {
  stop("mpa_name column was not retained in the final dataset. Check column standardization.")
}

# Plot
ggplot(data = combined_sf) +
  geom_sf(aes(fill = mpa_name)) +  # Color by MPA name
  theme_minimal()
...
# MPAs
mpa_sf <- read_sf(paste0(data_path,"spatial/cmar_capas/all_cmar_mpas.shp"))%>% 
  mpa_names()
...
sau_sf <- my_sf("SAU") %>% 
  ...
  mutate(
    label = gsub("\\(Pacific\\)", "", name),
    label = ifelse(label == "Galapagos Isl. (Ecuador)","Ecuador\n(Galapagos)",label)
  )


map_regions <-  ggplot() +
  geom_sf(data = sau_sf, aes(), fill = "grey90", color = "black") +
  # geom_sf(data = sau_sf, aes(fill = label), color = "black") +
  geom_sf(data = mpa_sf, aes(fill = region), color = "red", alpha = 0.5) +
  # geom_sf_label(data = sau_sf, aes(label = label), alpha = 0.5) +
  geom_label_repel(data = sau_sf,
                   aes(label = label,
                       geometry = geometry
                   ),
                   stat = "sf_coordinates",
                   ...
                   point.padding = 50,
                   hjust = "right",
                   nudge_x = ifelse(sau_sf$label == "Colombia ",1.5,
                                    ifelse(sau_sf$label == "Ecuador\n(Galapagos)",2,
                                           ifelse(sau_sf$label == "Panama ",2,
                                                  1)
                                    )
                   ),
                   nudge_y = ifelse(sau_sf$label == "Panama ",1,
                                    ifelse(sau_sf$label == "Colombia ",-2,
                                           ifelse(sau_sf$label == "Galapagos\n(Ecuador)",-2,
                                                  1)
                                    )),
                   alpha = 0.6,
                   size = 4
  ) +
  # geom_sf_label(data = mpa_sf, aes(label = region), alpha = 0.5) +
  my_land_map() +
  coord_sf(
    xlim = c(-95,-77),
    ylim = c(-5,9)
  ) +
  scale_fill_manual("Area Marina Protegida",
                    values = c(
