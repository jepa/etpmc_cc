...
# Distribution of per change
shapiro.test(stats_data$value)

#  For Area

area_stats <- stats_data %>%
  group_by(scen, period, ssp) %>%
  summarise(
    cor_test = list(cor.test(value, area_km2, method = "spearman")),
    .groups = "drop"
  ) %>%
  mutate(
    rho = map_dbl(cor_test, ~ .$estimate),     # Spearman rho
    p.value = map_dbl(cor_test, ~ .$p.value)  # p-value
  ) %>%
  select(scen, period, ssp, rho, p.value)
...
stats_data %>% 
  ...
  geom_smooth(method = "loess", se = TRUE, color = "black") +  # non-parametric trend line
  facet_grid(ssp ~ period, scales = "free_x") +
  labs(
    x = "Area (km²)",
  ...
  geom_text(
    data = area_stats,
    aes(
      x = min(stats_data$area_km2),
      y = max(stats_data$value),
      label = paste0("ρ = ", round(rho, 2), ", P = ", round(p.value, 3))
    ),
    inherit.aes = FALSE,
    hjust = 0,
    vjust = 1,
...
ggsave(
  paste0(results_path,"figures/relative_hist/all/sq_area_statistics.png"),
  last_plot(),
  height = 4,
  width = 8
)
...
# For Latitud
latitude_stats <- stats_data %>%
  group_by(scen, period, ssp) %>%
  summarise(
    cor_test = list(cor.test(value, lat, method = "spearman")),
    .groups = "drop"
  ) %>%
  mutate(
    rho = map_dbl(cor_test, ~ .$estimate),     # Spearman rho
...
ggsave(
  paste0(results_path,"figures/relative_hist/all/sq_lat_statistics.png"),
  last_plot(),
  height = 4,
  width = 8
)
...
fe_stats <- stats_data %>%
  group_by(scen, period, ssp) %>%
  summarise(
    cor_test = list(cor.test(value, fe, method = "spearman")),
    .groups = "drop"
  ) %>%
  mutate(
    rho = map_dbl(cor_test, ~ .$estimate),     # Spearman rho
...
### Area, latitude and fishing

#### Data for stats

```{r}

cmar_sq <- read.table("~/Library/CloudStorage/GoogleDrive-jepa88@gmail.com/My Drive/lenfest_mpa_project/data/01 spatial/compute_canada/mpa_sq.txt", quote="\"", comment.char="") %>% 
  mutate(
    index = row_number()
  )
...
# Test for  Evidence of linearity between variables

car::crPlots(mod_perm)

# Latitude, approximately linear relationship with the response, with the LOESS smooth closely matching the fitted linear trend. 
# Both area_km2 and fe noticeable curvature, suggesting potential nonlinearity in their effects.

mod_perm <- lmp(value ~ area_km2 + lat + fe, data = stats_data)
summary(mod_perm)


# Evidence of independence between observations 
...
# 
# data:  mod_perm
# DW = 1.675, p-value = 0.09703
# alternative hypothesis: true autocorrelation is greater than 0

# The test suggests no significant autocorrelation in the residuals (p > 0.05).

```

# GAM option

Because of non-linearity in area_km2 and fe

```{r}

library(mgcv)
mod_gam <- gam(value ~ s(area_km2) + s(lat) + s(fe),
               data = stats_data)
summary(mod_gam)
...
# 
# Formula:
# value ~ s(area_km2) + s(lat) + s(fe)
# 
# Parametric coefficients:
#             Estimate Std. Error t value Pr(>|t|)    
# (Intercept)  -6.0914     0.2094  -29.09 1.61e-12 ***
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# Approximate significance of smooth terms:
#               edf Ref.df      F  p-value    
# s(area_km2) 3.342  3.821  6.082 0.009299 ** 
# s(lat)      2.628  3.039 12.255 0.000612 ***
# s(fe)       1.000  1.000  3.999 0.068688 .  
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# R-sq.(adj) =  0.916   Deviance explained = 94.7%
# GCV = 1.4576  Scale est. = 0.87672   n = 20

# Interpretation:
# Area_km2 and lat have significant non-linear effects on the response variable (p < 0.01).
# fe has a marginally significant effect (p ~ 0.069), suggesting a potential linear relationship.
# The model explains a very high proportion of variance in the response (Adj. R² = 0.916).
...
summary(gam(value ~ s(area_km2) + s(lat) + fe, data = test_df))
```
Based on this result we fit a GAM where fe is linear and area_km2 and lat are smooth terms

```{r}

# Gam fro 1.26 at 2030

# List of SSPs and periods
ssps <- c("SSP1-2.6", "SSP5-8.5")
periods <- c("2030", "2050")

# Function to fit GAM and tidy results
fit_gam <- function(data, ssp_val, period_val) {
  # Correct filtering using .data
  df <- data %>% filter(.data$ssp == ssp_val, .data$period == period_val)
  
  mod <- gam(value ~ s(area_km2, k = 3) + s(lat, k = 3) + fe, data = df) #Note k = 3 bnecause os small data
  
  # Parametric terms
  param_tbl <- tidy(mod, parametric = TRUE) %>%
    mutate(term_type = "parametric")
  
  # Smooth terms
  smooth_tbl <- tidy(mod, parametric = FALSE) %>%
    mutate(term_type = "smooth")
  
  # Combine and add SSP/period info
  bind_rows(param_tbl, smooth_tbl) %>%
    mutate(SSP = ssp_val, period = period_val)
}

results_table <- expand.grid(ssp_val = ssps, period_val = periods) %>%
  pmap_dfr(~ fit_gam(stats_data, ..1, ..2)) %>% 
  mutate(
...
# GAM including scenarios and period

```{r}
# library(mgcv)

mod_spline <- gam(
  value ~ s(area_km2, k = 3) + s(fe, k = 3) + lat + ssp + period,
  data = stats_data,
  method = "REML"
)
