...
scenarios_data_abs <- read_csv(
  ...
  filter(esm != "mpis") %>% 
  bind_rows(
    mpismiss %>% mutate(year = as.numeric(year),
                        scen = ifelse(scen == "pp","rp",
                                      ifelse(scen == "pc","rc",scen
                                                    ))
    )
  )
...
[*Nicolas*] It is striking that for the GMR area, conservation measures only result in positive biomass changes, fisheries only also in + biomass changes, but when both are applied, there are negative effects on biomass... 


[*Response*] Thank you for this comment Nicolas, looking more into the data made me realize the actual patter happening. What is been seeing is that, in all cases, biomass in the Rules Implemented scenario is higher than in any other scenario, however, since climate change will have string impacts on the region, drops in the RI scenario will be larger, because it started with a larger biomass. Thoes that make sense? Basically, rules implemeneted increase biomass in the region but climate change impacts is regardless so the % reduction is larger, even thou the biomass continue to be higher. 
