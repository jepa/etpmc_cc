...
This script is used to transform the qualitative management scenarios into quantitative fishing effort scenarios for the DBEM. The scenarios are based on the information provided by the workshop participants and the management plans of the MPAs in the region.
...
scenarios <- read_excel(paste0(data_path,"02 management plans/scenarios_info.xlsx")) %>% 
  filter(fishing != "yes")

remove_mpa <- read_excel(paste0(data_path,"02 management plans/scenarios_info.xlsx")) %>% 
  filter(fishing == "yes")
...
# combined mpas
combined_sf <- read_sf(paste0(data_path,"01 spatial/cmar_capas/cmar_shp/all_cmar_mpas.shp"))
...
scenario_pannel <-
  bind_rows(
  sq_dbem_grid %>% mutate(scen = "Status quo (SQ)"),
  nr_dbem_grid %>% mutate(scen = "Regulations not implemented (RN)"),
  ri_dbem_grid %>% mutate(scen = "Regulations implemented (RI)"),
  rpp_dbem_grid %>% mutate(scen = "Regulations for fishing only (RF)"),
  rpc_dbem_grid %>% mutate(scen = "Regulations for conservation only (RC)")
) %>% 
  # View()
  ggplot() +
  geom_tile(
  ...
  geom_sf(data = sau_sf, aes(), fill = "transparent", color = "red")  +
  geom_sf(data = combined_sf, aes(), fill = "transparent", color = "black")  +
  MyFunctions::my_land_map(country_selection = NULL) +
  my_ggtheme_m("Reg",facet_tl_s = 8,ax_tl_s = 6,ax_tx_s = 4,leg_tl_s = 4,leg_tx_s = 4) +
  labs(x = "Latitude",
       y = "Longitude") +
  coord_sf(
    xlim = c(-95,-77),
...
ggsave(
  paste0(data_path,"/03 results/scenarios_paper/supplements/scenario_pannel.png"),
  scenario_pannel,
  width = 7,
  height = 5)
